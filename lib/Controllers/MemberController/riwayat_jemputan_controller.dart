...
class OrderJemputanController extends GetxController {
  ...
  Future daftarOrderJemputan() async {
    Authorization = await GetStorage().read(STORAGE_TOKEN);
    final respone = await http.get(Uri.parse(ApiUrl.daftarOrderJemputan+'/${filter.value}'),
        headers: {'Authorization': Authorization});
    print(
    ...
    if (respone.statusCode == 401) {
      ...
      kategoriFilter.value = await List<Liststatus>.from(hasil['liststatus'].map((x) => Liststatus.fromJson(x))).obs;
      kategoriFilter.value.add(allItem);
  ...
  Future nextDaftarOrderJemputan(_page) async {
    ...
    final respone = await http.get(
        // Uri.parse("${ApiUrl.daftarOrderJemputan}/${filter.value}/${_page}"),
        Uri.parse(urlnext.value),
        headers: {'Authorization': Authorization});
    print(
        "------------------------DATA daftar Order jemputan Sampah ${_page}--------------------------");
    print(respone.statusCode);
    print(json.decode(respone.body));
    modelDaftarOrderJemputan = modelDaftarOrderJemputanFromJson(respone.body);
    if (respone.statusCode == 401) {
